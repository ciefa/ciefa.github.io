...
The distributed application is a single offline `index.html` with no runtime installation. Open it directly from disk in current Chrome/Chromium or Firefox. Development uses a deterministic build to embed the text model, bounded parser worker, and dependency licenses. Changes to `src/roundtrip/` require regenerating `index.html`.
...
Playwright is pinned to 1.57.0 in the lockfile. The build also pins parse5 8.0.0, entities 6.0.1, and esbuild 0.28.2. Parser/dependency licenses are retained verbatim in `LICENSES/` and embedded in saved copies. `check:roundtrip-build` fails without writing if generated artifacts or shared limits disagree. Its browser builds are Chromium 143.0.7499.4 (revision 1200), Firefox 144.0.2 (revision 1497), and WebKit 26.0 (revision 2227). On Linux, use a distribution supported by Playwright's dependency installer or supply the corresponding browser libraries for your distribution. Browser downloads and operating-system libraries are not application dependencies.
...
npx playwright test tests/performance.spec.cjs tests/roundtrip-performance.spec.cjs --project=chromium --reporter=json > playwright-report/performance.json
```
